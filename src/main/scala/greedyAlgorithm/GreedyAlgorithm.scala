...
class GreedyAlgorithm {
  ...
  //どういう順番で仕事をこなすかを格納する
  var resultTaskNumList :mutable.MutableList[Int] = mutable.MutableList.empty[Int]
  def execute():Unit = {
    //終了時間にindexをつけてソートする
    ...
    //最初の仕事を結果に格納
    resultTaskNumList = resultTaskNumList :+ firstTaskIndex
    //下のfor文で使う。前の仕事の終了時間
    ...
    println(resultTaskNumList.length)
    resultTaskNumList.foreach(result=>println(s(result),t(result)))
  }
  ...
  private def resolveTask(eachT:(Int,Int), preTTime:Int):Int = {
    ...
    if (s(tIndex) > preTTime){
      resultTaskNumList = resultTaskNumList :+ tIndex
      return tTime
    }
    preTTime
  }
